...
an ellipse in general, thus only two dimensions. Nonetheless, in the beginning, i.e., for small $t$, the
...
\begin{figure}
  \centering
    (a) \raisebox{-55mm}{\includegraphics[scale=0.45]{fig/spectrum.pdf}} \hfill
    (b) \raisebox{-55mm}{\includegraphics[scale=0.45]{fig/ellipse.pdf}} \\[5mm]
    (c) \raisebox{-55mm}{\includegraphics[scale=0.45]{fig/wave.pdf}}
  \caption{Dynamic system behavior of a pure random reservoir with $N^\mathrm{res} = 100$ neurons:
    (a) Eigenvalue spectrum of the reservoir matrix $W^\mathrm{res}$.
    (b) In the long run, we get an ellipse trajectory, thus only two dimensions.
    (c) Projected to one dimension, we have pure sinusoids with one
	single angular frequency, sampled in large steps.}
  \label{ell}
\end{figure}
...
\cref{output}).\opt{long}{ Depending on the relation between the overall network
size $N$ and the time series length $n$, we observe:
\begin{enumerate}
...
\begin{algorithmic}[1]\State
	...
	$s = \left[ \begin{array}{c} S(0) \\ r_0 \end{array} \right]$ where
		$r_0 = \frac{1}{\sqrt{N^\mathrm{res}}} \cdot
...
precision (cf. \cref{approx}), not every function can be implemented by LRNNs,

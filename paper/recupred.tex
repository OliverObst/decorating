...
\address{$^{1}$Faculty of Automation and Computer Science, Harz University of Applied Sciences,\\
...
the previous state-of-the-art for the MSO task with a minimal number of units.  \absbreak
\end{abstract}
...
compression, i.e., the compact representation of time series, for example, by a function 
$f(t)$, and prediction of future values.

Numerous studies address these topics using RNNs, particularly variants of 
networks with long short-term memory (LSTM) \citep{HS97}. Recent works in this area also aim to enhance the performance of RNNs, for example, HiPPO \citep{GDE+20} and structured state spaces \citep{GGR22}, which learn representations of signals over time.
...
which allows for network size reduction; no backpropagation or gradient-descent 
...
One variant of ESNs combines several independent
(decoupled) smaller networks \citep{XYH07}. ESN-style (random) initialization has been shown
...
State-frequency memory (SFM) RNN \citep{HQ17}  
...
\citep{MM+19} propose a method that estimates the contribution of a neuron to
...
\begin{proof}
See Appendix \ref{proof:exponential}.
\end{proof}
...
Michael et al.~\citep{MO+19} describe a research dataset using some of the released binaries of
...
considering only the $(x,y)$-coordinates of the ball and the 22 players.
...
\caption{Left: Ball trajectory of RoboCup 2D soccer simulation game \#6
...
Right: Dots (in blue) mark positions

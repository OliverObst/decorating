...
\begin{figure}
  ...
  \caption{Dynamic system behavior of a pure random reservoir with $N^\mathrm{res} = 100$ neurons:
    ...
    (b)~Visualization of $f(t)$ by planar projection. For large $t$, we get an
	ellipse trajectory, thus only two dimensions (cf. \cref{twodim}).
    (c)~Projected to one (arbitrary) dimension, in the long run, we have pure
	sinusoids with one single angular frequency, sampled in large steps.}
  \label{ell}
\end{figure}

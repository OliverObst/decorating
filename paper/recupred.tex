...
\begin{enumerate}
  ...
  \item There is no distinction between input and output but only one (joint)
	group of $N^\mathrm{in}=N^\mathrm{out}=d$ input/output neurons. They may
...
In fact, LRNNs can model polynomials (cf. \cref{parasin}, parabola), general
...
\begin{itemize}
  \item Because of the linear activation, the spectral radius of the reservoir
	weights matrix $W^\mathrm{res}$ can be set to $1$ (cf. \cref{thedef}).
...
increasing. This holds for the Fibonacci series ($\lambda_1>1$).

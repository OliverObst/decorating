...
The topic of learning all weights in the matrix $W$ has been investigated
...
Jordan matrix $J$ (line 24). The subroutine $\mathrm{Out}(J_I)$ (lines 25-31)
...
\citep{DDH07}. Therefore, the time complexity is just $O(N^3\log(N))$ for both output
...
$O(N^3\log(N))$ for LRNNs does not look overly expensive. It is the
...
sinusoids. We concentrate here solely on the most complex case $K=8$ (cf. \cref{signal})
...
\begin{figure}
  ...
  \includegraphics[width=0.65\textwidth]{fig/mso8_prediction_1} % ,height=5.5cm
...
steps, LRNNs using 600 neurons achieve an average RMSE of
...
performance for all approaches often is not much better than the baseline (cf.
\cref{stockfig}). In line with that, the review by \citet{SIZ19} shows that
...
\begin{figure}
  \centering
  \includegraphics[width=0.65\textwidth]{fig/stock_prediction_fme} % ,height=5cm

...
to all neurons including the input/output neurons, thus the output from the
...
\cref{diff}. In consequence, according to \cref{rem} and because $\exp(0)=\dot{\exp}(0)=1$,
...
with unit spectral radius, and (c)~all eigenvalues are distinct (which is almost
...
into real ones (cf.~\cref{real}): Obviously, all values of $f(t)$ are
(a)~real-valued (because $W^\mathrm{res}$ and the start vector $s$ are
completely real-valued) and (b)~are linear combinations of the two complex
conjugate eigenvectors $v = v_1$ and $\overline{v} = v_2$ in this case. Let us
consider the vectors $\ell(\kappa) = \kappa v + \overline{\kappa}\,\overline{v}$
for $\kappa \in \mathbb{C}$ with $|\kappa| = 1$, which share both properties,
and determine the vectors with extremal lengths among them. Let $\kappa =
e^{\mathfrak{i} \varphi} = \cos(\varphi) + \mathfrak{i} \sin(\varphi)$ (Euler's
formula) and $v_\Re$ and $v_\Im$ be the real and imaginary parts of $v$,
respectively, i.e., $v = v_\Re+\mathfrak{i} v_\Im$ and $\overline{v} =
v_\Re-\mathfrak{i} v_\Im$. Then, for the square of the vector lengths, it holds:
\begin{eqnarray*}
\|\ell(\kappa)\|^2
...
\end{eqnarray*}
In order to find out the vectors $\ell(\kappa)$ with extremal lengths,
...
for $\kappa$ that are orthogonal to each other: $\kappa_1 =
e^{\mathfrak{i}\varphi}$ and $\kappa_2 = e^{\mathfrak{i}(\varphi+\pi/2)}$.
$\ell(\kappa_1)$ and $\ell(\kappa_2)$ represent the main axes of an ellipse.
...
trajectory (\cref{twodim}) is almost zero already after a few hundred steps (cf.
\cref{asymptot}). Of course this depends on the eigenvalue distribution of the
transition matrix \citep{TV10}. So the long-term behavior may be different for
transition matrices other than pure random reservoirs.

\begin{figure}[t]
  \centering
    (a) \raisebox{-55mm}{\includegraphics[scale=0.4]{fig/spectrum}} \hfill
    (b) \raisebox{-55mm}{\includegraphics[scale=0.4]{fig/ellipse}} \\[5mm]
    (c) \raisebox{-55mm}{\includegraphics[scale=0.4]{fig/wave}}
...
|z|$ for all $z \in \mathbb{R}$ but the convergence may be rather slow, but it
guarantees contractivity and hence for any fixed input (not just the origin) the
system converges to a unique fixed point. This leads

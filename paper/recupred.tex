...
matrix $W$. We will come back to this later (in \cref{continued}).
\end{exmp}

\begin{figure}
  (a) \raisebox{-6cm}{\includegraphics[width=0.4\textwidth]{fig/example.pdf}}
	\hfill
  (b) \raisebox{-4cm}{\includegraphics[width=0.4\textwidth]{fig/fibonacci.pdf}}
  \caption{LRNNs for (a) $f(t) = t^2$ and (b) the Fibonacci series
	($0,1,1,2,3,5,8,\dots$) with time step $\tau=1$. The input/output neuron
	$x_1$ is marked by a thick border. The initial values of the neurons at
...
(\cref{binet}), which hence is derived numerically by the LRNN learning

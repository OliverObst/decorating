...
In \citep{ZMV+25}, the authors establish an explicit connection and a direct architectural match between Koopman operator approximation and linear RNNs, with the goal of  time-series prediction.
...
(Fisher) memory of the network. Furthermore, \citep{CW+16} investigates the
asymptotic performance of linear ESNs from a solely theoretical point of view.

\subsection{Recurrent Neural Networks}

Simple RNNs have been proposed originally in \citep{Elm90}. By allowing them to accept sequences
...
some of their properties (like short-term memory) are already investigated in
...
a prominent and dominant type of RNN. The expression \enquote{long
...
\citep{CSB21} proposes an approach to address memorization challenges in RNNs
...
\citet{GGR22,OS+23} cover ideas of linear recurrent equations in so-called state
...
for neural networks in \citep{Ree93}. More recent work \citep{LAT19} presents 
...
\citep{MM+19} proposes a method that estimates the contribution of a neuron to
...
For feedforward neural networks, the forward-forward algorithm \citep{Hin22}
presents an approach to training models without backpropagation but, at least in
its current form, is only applicable to the classification of static patterns.
...
autoregressive models and LRNNs, have been studied at length \cite{Ham94}.
Autoregressive frameworks are also common in current machine-learning
applications like language modeling, e.g., in the generative pre-trained
transformer GPT-3 and their successors,
by next word prediction \citep{BM+20}. The models applied in this context,
however, are very complex (175 billion parameters and much more) and non-linear.
...
the rotation speed can be derived from the eigenvalue $\lambda_1$. In each step
...
\opt{long}{\begin{remk}[generalization to nonlinear activation]
...
straightforward manner to nonlinear, invertible activation functions $g$.
...
learning procedure together with some case studies (cf. \cref{result})%
\opt{unblind}{is publicly available at \url{http://github.com/OliverObst/decorating/}.}%
\opt{blind}{ will be made available upon acceptance.}
...
\begin{table}[t]
  ...
  \caption{Percentages of correct predictions of the last element for 20 number
	...
	$N^\mathrm{res}=7$ reservoir neurons; \textbf{(c)}~same procedure but in
	addition always the previous time series value is used as clue.}
  \label{table}
\end{table}
...
regular game play, the so-called \enquote{play-on} mode.
...
\cref{complexity}). Thus the LRNN model is definitely smaller than the original
time series representation of a game. The complete learning procedure runs in
less than a minute on standard hardware.

...
% versioning: long,proof,accepted
...
kind of RNN together with their properties, including the general network 
...
The Legendre memory unit (LMU) \citep{VKE19}  is a memory cell for recurrent neural networks that maintains information across long windows of time using relatively few resources. It is derived from the linear transfer function for
...
\begin{figure}
  \centering
    (a) \raisebox{-35mm}{\includegraphics[scale=0.23]{fig/spectrum}} \hfill
    (b) \raisebox{-35mm}{\includegraphics[scale=0.23]{fig/ellipse}} \hfill
    (c) \raisebox{-35mm}{\includegraphics[scale=0.23]{fig/wave}}
...
of single linkage clustering \citep{GR69}. \opt{accepted}{\def\showlink{}}
The complete implementation of the learning procedure together with some case studies (cf. \cref{result}) 
\ifdefined\showlink
    is publicly available at \url{http://github.com/OliverObst/decorating/}.
\else
...
For our experiments we evaluated ten games of the
top-five teams in a public research dataset \citep{MO+19} that contains recorded games 
from the RoboCup 2D soccer simulation league \citep{CDF+03,GFG16} in 2016 and 2017 \citep{MO+18}\opt{accepted}{ (Data set available from \url{http://bitbucket.org/oliverobst/robocupsimdata})}.  We consider the $(x,y)$-coordinates of the ball and all 22 players for all time points during regular game play.
%the so-called \enquote{play-on} mode.
...
also \cref{stockfig}). In line with that, the review in \citep{SIZ19} shows that
...
some extent also in \citep{CW+16}. Last but not least, other machine learning tasks besides

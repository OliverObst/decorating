...
straightforward manner to nonlinear, invertable activation functions $g$ as
follows: Clearly, $g$ has to be applied component-wise to \cref{Res} while
running the input through the reservoir and to \cref{linear}. The solution of
the resulting equation $Y^\mathrm{out} = g\big(W^\mathrm{out} \cdot X\big)$ is
then $W^\mathrm{out} = g^{-1}\big(Y^\mathrm{out}\big)/X$. Hence LRNN learning is
as easy as learning a single-layer perceptron. We can approximate any (possibly
nonlinear) function over time $f(t)$ efficiently. 
\end{remk}

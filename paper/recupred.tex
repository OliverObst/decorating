...
\usepackage[long,proof]{optional}
...
(Fisher) memory of the network. Furthermore, work in \citep{CW+16} investigates the
...
Work in \citep{CSB21} proposes an approach to address memorization challenges in RNNs
...
network activity reduces to at most two dimensions -- independent from
...
predicting the arithmetic mean of the training data, ARIMA \citep{HK08}, simple sparse ESN demo by Mantas Luko\v{s}evi\v{c}ius from \url{http://mantas.info/code/simple_esn/}, and LSTM forecasting model implemented in Darts with hyperparameter optimization.
\if 0
\begin{itemize}
  \item Baseline: Constantly predict the arithmetic mean of the training data.
  \item ARIMA: We use the AutoARIMA package \citep{HK08}.
...
\cref{robocup}). Note that the size of the learned network is in $O(N)$. % (cf. \cref{complexity}). 
Thus the LRNN model is definitely smaller than the
...
The work in \citep{Lit20} analyzes $30$ different stocks by ARIMA and LRNNs using the
...
\begin{figure}
  \centering
  \includegraphics[width=0.58\textwidth]{fig/stock_prediction_fme}

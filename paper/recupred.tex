...
|\lambda_1|\,e^{\mathfrak{i}\omega\tau}$. The two-dimensional ellipse trajectory
...
function $f(x)$ from the (possibly multi-dimensional) input $x$. Only linearly
...
easy as learning a single-layer perceptron an allows us to approximate any
(possibly nonlinear) function over time $f(t)$ efficiently. 
\end{remk}

\cref{approx} is thus related to the \emph{universal approximation theorem} for
...
0.311, 0.420, 0.510, 0.630, 0.740, 0.850, 0.970 \big\}$ their frequencies.
\end{exmp}
...
LRNN are also advantageous considering the time required to train a well performing model: On average, this takes less than 3 s per series, excluding file access. The ARIMA experiments were fastest (0.35 s / series), and ESN took 1.24 s / series. LRNN extend the ESN approach with our approach, and take 2.97 s on average, this time includes the repeated reservoir generations in our approach. LSTM including hyper parameter selection took 125.4 s / series.% (all run on an Intel i9-10940X, 3.3GHz, 128GB RAM).
%%% RESULTS (times)
...
\]
From this, we can read off the desired transition matrix $W$ from \cref{mat}.

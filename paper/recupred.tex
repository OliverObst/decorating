...
\newcommand{\commt}[1]{\%~#1\\}%
\begin{tabular}{rl}
1 & \commt{$d$-dimensional function, given sampled, as time series}
2 & $S = [ f(0) \dots f(n) ]$\\ \\
3 & \commt{random initialization of reservoir and input weights}
4 & $W^\mathrm{in} = \mathrm{randn}(N,d)$\\
5 & $W^\mathrm{res} = \mathrm{randn}(N^\mathrm{res},N^\mathrm{res})$\\ \\
6 & \commt{learn output weights by linear regression}
7 & $X = \left[W^t \cdot s\right]_{t=0,\dots,n}$\\
8 & $Y^\mathrm{out} = \big[ S(1)\ \cdots\ S(n) \big]$\\
9 & $W^\mathrm{out} = Y^\mathrm{out}/X$\\ \\
10 & \commt{transition matrix and its decomposition}
11 & $W = \left[ \begin{array}{cc} \multicolumn{2}{c}{W^\mathrm{out}}\\ W^\mathrm{in} & W^\mathrm{res} \end{array} \right]$\\
12 & $J = \mathrm{jordan\_matrix}(W)$\\ \\
13 & \commt{network size reduction}
14 & $y = \big[ 1 \cdots 1 \big]^\top$\\
15 & $Y = \left[J^t \cdot y\right]_{t=0,\dots,n}$\\
16 & $A = X/Y$ with rows restricted to input/output dimensions\\
17 & reduce($A,J,y$) to relevant components\\
...
series $f(0),\dots,f(n)$ as input $S$ (lines 1-2) and generate a random
reservoir of size $N^\mathrm{res}$ (lines 3-5). After learning the output
weights $W^\mathrm{out}$ (lines 6-9) and decomposing the resulting transition
matrix $W$ (lines 10-12), the network size reduction procedure (lines 13-17)
...
trials, and the top 3\% are the best networks with respect to their RSME. Thus,
...
Note that always the spectral radius of the reservoir weights matrix
$W^\mathrm{res}$ remains $1$ all the time. Only the spectral radius of the
overall transition matrix $W$ (according to \cref{matrix}) of size $n+1$ may be
greater than $1$ after learning the output weights $W^\mathrm{out}$. This is the
case for the Fibonacci series because it is exponentially increasing
($\lambda_1>1$).

...
We will see (in \cref{infty}) that often the network dynamics may be reduced to a
...
\begin{figure}[h]
  \centering
    (a) \raisebox{-55mm}{\includegraphics[scale=0.4]{fig/spectrum.pdf}} \hfill
    (b) \raisebox{-55mm}{\includegraphics[scale=0.4]{fig/ellipse.pdf}} \\[5mm]
    (c) \raisebox{-55mm}{\includegraphics[scale=0.4]{fig/wave.pdf}}
...
Since \cref{linear} is equivalent to simultaneously solving the equations $w_k
\cdot X = y_k$, for $1 \le k \le N$ where $w_1,\dots,w_N$ and $y_1,\dots,y_N$
denote the row vectors of the matrices $W^\mathrm{out}$ and $Y^\mathrm{out}$,
...
Thus, in theory, any time-dependent function can be approximated with arbitrary
precision, even continued on input other than nonnegative integer numbers (cf.
\cref{general}), although clearly not every function can be implemented by
LRNNs, in particular functions increasing faster than single-exponential (cf.
\cref{exponential}) like $2^{2^t}$ (double-exponential) or $t\/!$ (factorial
function). Also in practice, the LRNN learning procedure performs rather well
(cf. \cref{result}). Nevertheless, the matrix $X$ may be ill-conditioned,
because the reservoir state sequence as part of the matrix $X$ reduces to at
most two dimensions for large $t$ -- independent of the number of reservoir
neurons (cf. \cref{ellipse}). Hence, the rank of the matrix $X$ may be not
maximal and in consequence \cref{linear} may not be solvable numerically
(although we may have an equation system with the same number of equations and
unknowns). A simple increase of the number of reservoir neurons does not help
much.

However, one could learn not only the output weights $W^\mathrm{out}$ as in ESNs
but the complete transition matrix $W$: For this, let $\big[R(0) \cdots R(n)
...
have to solve the linear matrix equation $W \cdot X = Y$ (cf. \cref{linear})
with $X$ as in \cref{Xin} and:
\[ Y = \left[ \begin{array}{ccc}
	S(1) & \cdots & S(n)\\
	R(1) & \cdots & R(n)
\end{array} \right] \]
By this, $W^\mathrm{in}$ and $W^\mathrm{res}$, i.e., the input and reservoir
weights are learned, not only the output weights in $W^\mathrm{out}$. But our
experiments indicate that this procedure is less reliable than the one with
given, i.e., predefined random input and reservoir weights and unit spectral
radius for the reservoir (cf. \cref{output}).

The topic of learning all weights in the matrix $W$ has also been investigated by
\citet{PDW13} for ESNs with nonlinear activation function. However, for LRNNs, the
...
matrix $W$ (lines 11-13), the network size reduction procedure (lines 14-19)
...
\includegraphics[width=\textwidth]{fig/mso8_prediction_1} % ,height=5.5cm
  \caption{Example MSO8 graph that will still be changed.}
  \label{arima}
\end{minipage}
  \hfill%
\begin{minipage}[t]{0.48\textwidth}
 \includegraphics[width=\textwidth]{fig/stock_prediction_fme} % ,height=5cm

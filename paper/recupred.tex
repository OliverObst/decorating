...
Let us illustrate the LRNN learning procedure with the Fibonacci series
...
$N^\mathrm{res}$. Note that always the spectral radius of the reservoir weights
matrix $W^\mathrm{res}$ remains $1$ all the time. Only the spectral radius of
the overall transition matrix $W$ (according to \cref{matrix}) of size $n+1$
may be greater than $1$ after learning the output weights $W^\mathrm{out}$. This
is the case for the Fibonacci series because it is exponentially increasing. The
network size reduction procedure often yields minimal networks with only two
reservoir neurons representing
\[
...
(\cref{binet}). For instance, for $N^\mathrm{res}=n=30$, we obtain minimal
networks in 32\% of the cases from $100$ trials, and the top 3\% are the best
networks with respect to their RSME. Thus, by employing a standard validation
procedure, the LRNN in \cref{examples}\,c actually can be derived numerically by
the LRNN learning procedure with network size reduction.
\end{exmp}
